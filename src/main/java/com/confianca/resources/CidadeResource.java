...
import com.confianca.services.CidadeService;
...
public class CidadeResource {
    ...
    public ResponseEntity<Void> insert(@RequestBody @Valid Cidade obj) {
    ...
    public ResponseEntity<List<Cidade>> findByEstadoId(@PathVariable Integer id) {
        List<Cidade> list = service.findByEstadoId(id);
        return ResponseEntity.ok().body(list);
    }

    @PreAuthorize("hasAnyRole('ADMIN')")
    @RequestMapping(value = "/pais/{id}",method = RequestMethod.GET)
    public ResponseEntity<List<Cidade>> findByEstadoPaisId(@PathVariable Integer id) {
        List<Cidade> list = service.findByEstadoPaisId(id);
        return ResponseEntity.ok().body(list);
    }
