...
public class RecebimentoResource {
    ...
    public ResponseEntity<Recebimento> find(@PathVariable Integer id) {
        Recebimento obj = service.find(id);
        return ResponseEntity.ok().body(obj);
    }

    @PreAuthorize("hasAnyRole('ADMIN')")
    @RequestMapping(method = RequestMethod.POST)
    public ResponseEntity<Void> insert(@RequestBody @Valid Recebimento obj) {
    ...
    public ResponseEntity<Void> update(@Valid @RequestBody Recebimento obj, @PathVariable Integer id) throws CieloRequestException, IOException {
        obj.setId(id);
    ...
    public ResponseEntity<Page<Recebimento>> findPage(
            @RequestParam(value = "page", defaultValue = "0") Integer page,
            @RequestParam(value = "linesPerPage", defaultValue = "24") Integer linesPerPage,
            @RequestParam(value = "orderBy", defaultValue = "nome") String orderBy,
            @RequestParam(value = "direction", defaultValue = "ASC") String direction) {
        Page<Recebimento> list = service.findPage(page, linesPerPage, orderBy, direction);
        return ResponseEntity.ok().body(list);
    }
}
